...
const CategoryPiechart = ({ dateRange, categories, transactions }: Props) => {
  const spendingPerCategory = new Map<number | null, number>();
  categories.forEach((category: Category) => {
    spendingPerCategory.set(category.id, 0);
  });
  transactions
    ...
    .forEach((transaction: Transaction) => {
      if (spendingPerCategory.has(transaction.categoryId ?? null)) {
        const current = spendingPerCategory.get(
